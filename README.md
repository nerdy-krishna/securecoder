...
> **Status:** v0.5.0 — `/securecoder-setup`, `/securecoder-scan` (full SAST suite + reports + trend), and `/securecoder-fix` (SAST findings with safety loop + commit-per-fix + restore) are functional today. The other four skills ship slice-by-slice. See [docs/issues/](docs/issues/) for the implementation backlog.

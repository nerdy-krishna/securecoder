...
> **Status:** v0.3.0 — `/securecoder-setup` and `/securecoder-scan` (full SAST suite: Semgrep + Bandit + Gitleaks + OSV-scanner) are functional today. The other five skills ship slice-by-slice. See [docs/issues/](docs/issues/) for the implementation backlog.
...
| `/securecoder-scan` | Audit existing code with SAST tools and/or LLM-driven compliance review. | **v0.3.0 (multi-tool SAST: Semgrep, Bandit, Gitleaks, OSV-scanner); compliance pass in v0.7.0** |

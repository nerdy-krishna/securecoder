...
> **Status:** v0.4.0 — `/securecoder-setup` and `/securecoder-scan` (full SAST suite: Semgrep + Bandit + Gitleaks + OSV-scanner) are functional today, with markdown + self-contained HTML reports and cross-run trend tracking. The other five skills ship slice-by-slice. See [docs/issues/](docs/issues/) for the implementation backlog.
